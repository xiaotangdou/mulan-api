...
function screenResponse(response) {
  ...
  if (response.status !== 200 && response.status !== 204) {
    throw new MlApiError({
      code: MlApiErrorCode.PROT_RESP_MALFORMAT_DATA,
...
function getAdapterConfigFromApiConfig(apiConfig) {
  ...
  /** remove those LcApi-specific configs that would not be used for axios */
...
/**
 * Axios adapter as data communication layer for LcApi
 */
class LcApiAdapterAxios {
  constructor() {
  ...
  _initiate(runningConfig) {
    return this._axios(runningConfig)
      .catch((error) => {
...
export default LcApiAdapterAxios
